...
class State(BaseState):
    ...
    def __init__(self, views, labs, start_idx, opts): 
        """
            views: (B, N, M, F)  array
            labs: (B, ) numpy array
    ...
    def reward_fn(self, prediction_probs):
        ...
        out_rewards[self.labs.cpu().numpy() == np.argmax(prediction_probs, axis=1)] = 1
        # out_rewards = out_rewards * np.max(prediction_probs, axis=1)
        return out_rewards
