...
def get_starts(N, M, batch_size, option):
    ...
    if option == 0:
        ...
        ##################################adding mask####################################################
        start_idx = [[2, random.randint(0, M-1)] for i in range(batch_size)]
        ##################################adding mask####################################################
    else:
        start_idx = [[N//2,M//2-1] for i in range(batch_size)]
    return start_idx

def get_target_starts(N, M, batch_size, support_start_idx):
    """
    Given the number of elevations(N), azimuths(M), batch size and the option (different types of starts),
    this function returns the start indices for the batch.
    start_idx: list of [start_elev, start_azim] for each panorama in the batch
    """
    # pool = [[i,j] for i in range(N) for j in range(M)]
    start_idx = []
    for i in range(len(support_start_idx)):
        pool = [[i,j] for i in range(N) for j in range(M)]
        ##################################adding mask####################################################
        pool = [[2,j] for j in range(M)]
        ##################################adding mask####################################################
        pool.remove(support_start_idx[i])
        rng = np.random.RandomState()
        # keys = rng.choice(len(pool), size=15, replace=False)
        ##################################adding mask####################################################
        keys = rng.choice(len(pool), size=15, replace=True)
        ##################################adding mask####################################################
        # [start_idx.append(pool[k]) for k in keys]
        start_idx.extend([pool[k] for k in keys])
    # start_idx = [[random.randint(0, N-1), random.randint(0, M-1)] for i in range(batch_size)]
