...
def augment_image(image, k, channels, augment_bool, args, dataset_name):
    ...
    if len(image.shape) > 3:
        ...
        output_images = [[] for i in range(image.shape[0])]      
        for i in range(image.shape[0]):
            for j in range(image.shape[1]):
                if augment_bool is True:
                    for transform_current in transform_train:
                        images = transform_current(image[i,j,:,:,:])
                    output_images[i].append(images)
                else:
        ...
        for k in range(image.shape[0]):
            output_image.append(torch.stack(output_images[k]))
        image = torch.stack(output_image)
    else:
        if augment_bool is True:
...
class FewShotLearningDatasetParallel(Dataset):
    ...
    def load_test_image(self, filepath):
        ...
        try:
            ...
            image = scipy.io.loadmat(filepath,verify_compressed_data_integrity=False)
    ...
    def get_data_paths(self):
        ...
        for i in range(len(data_image_path_list_raw)):
            image_file = data_image_path_list_raw[i]
            label = self.get_label_from_path(image_file)
            data_image_path_dict[label_name_to_idx[label]].append(image_file)        
    ...
    def load_image(self, image_path, channels):
        ...
        """
        # print(image_path)
        image = scipy.io.loadmat(image_path,verify_compressed_data_integrity=False)
        image = image['a']
    ...
    def load_batch(self, batch_image_paths):
        ...
        # image_batch = np.tile(image_batch,self.args.image_channels)
        return image_batch
    ...
    def get_set(self, dataset_name, seed, augment_images=False):
        ...
        ##################################Inter-class instance task############################################
        selected_classes = rng.choice(list(self.dataset_size_dict[dataset_name].keys()),
                                      size=1, replace=False)
        selected_classes = np.array(list(selected_classes) * self.num_classes_per_set)
        ##################################Inter-class instance task############################################        
        ...
        for class_entry in selected_classes:
            ...
            choose_samples_list = rng.choice(self.dataset_size_dict[dataset_name][class_entry],
                                             size=1, replace=True)
            choose_samples_list = np.array([choose_samples_list] * (self.num_samples_per_class + self.num_target_samples)).squeeze()
            ##################################Inter-class instance task############################################        
            class_image_samples = []
            class_labels = []
        ...
        ##################################Inter-class instance task############################################
        for i in range(self.num_classes_per_set):
            support_set_labels[i] = np.array(i*self.num_samples_per_class,dtype=np.float32)
            target_set_labels[i] = np.array([i]*self.num_target_samples,dtype=np.float32)
        ##################################Inter-class instance task############################################
